...
画法：左侧输入，中间由小代或怪机器处理，右侧输出，橙色箭头表达主流向。

### 系统局部

适合：信息来源、过滤器、数据库、渲染器、agent 系统局部。

画法：只画 3-5 个核心模块，小代参与其中一个关键动作。
...
画法：一层层盒子，不要正式金字塔；小代亲手搬运或搭建。

### 地图路线

适合：从想法到上线、用户路径、内容承接路径、学习路线。

画法：一条弯曲路径，少量节点，小代牵线或走路。
...
3. 让小代承担动作：不是站旁边，而是操作机器、拉线、守门、搬运、修补、称重、扶梯子、记录或把东西塞进某个怪装置。小代不能变形成机器、漏斗、洞口或黑块。
...
不要默认打开或复刻 `assets/examples/`。旧图片只用于理解线条密度、留白和颜色克制，不得用作角色身份参考。小代身份只以 `assets/character-reference/xiaodai-reference-v1.png` 为准。
